...
import { mockProducts } from '@/views/mock/home'
...
async function search() {
  ...
  try {
    ...
    results.value = Array.isArray(result) ? result : (result.list || result.records || [])
  } catch (e) {
    try {
      const result = await getProductPage({ page: 1, size: 20, keyword: keyword.value })
      results.value = result.list || result.records || mockProducts
    } catch {
