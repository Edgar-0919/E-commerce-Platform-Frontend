        ...
        >
          ...
          <div class="col-info">
            ...
            <div class="item-detail">
              ...
              <p class="item-spec">{{ item.spec || '规格：默认' }}</p>
            </div>
          </div>
          <div class="col-price">
          ...
          <div class="col-subtotal">
            <span class="subtotal-text">¥{{ item.price * item.quantity }}</span>
          </div>
...
async function loadCart() {
  ...
  try {
    ...
    cartList.value = result.length > 0 ? result : mockCartItems
    selectedItems.value = cartList.value.map(item => item.id)
  } catch (e) {
    cartList.value = mockCartItems
    selectedItems.value = cartList.value.map(item => item.id)
  } finally {
    loading.value = false
  }
