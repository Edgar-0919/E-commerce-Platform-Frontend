<template>
  <div class="home-page">
    ...
    <div class="quick-entries">
      <div
        v-for="entry in quickEntryItems"
        :key="entry.name"
        class="quick-entry-item"
        @click="handleSearch(entry.name)"
      >
    ...
    <div class="seckill-section">
      <div class="seckill-header">
        <div class="seckill-header-left">
          <span class="seckill-brand">天猫秒杀</span>
...
import {
  carouselItems, hotTags, quickEntries,
  mockProducts, mockDigitalProducts, mockHomeProducts
} from '@/mock/home'
...
const productSections = computed(() => {
  ...
  const list = products.value.length > 0 ? products.value : mockProducts
  const shuffled = [...list].sort(() => Math.random() - 0.5)
  const extraProducts = [...mockDigitalProducts, ...mockHomeProducts].sort(() => Math.random() - 0.5)
  return [
    { title: '猜你喜欢', category: 'recommend', products: shuffled.slice(0, 10) },
    { title: '为你推荐', category: 'suggest', products: extraProducts.slice(0, 10) }
  ]
})
...
async function loadProducts() {
  ...
  error.value = false
  try {
    const result = await getProductPage({ pageNum: 1, pageSize: 15 })
    products.value = result.list || mockProducts
  } catch (e) {
...
.seckill-header {
  ...
  align-items: center;
  padding: var(--spacing-md) var(--spacing-lg);
  background: linear-gradient(90deg, #fff2eb, #fff);
}
